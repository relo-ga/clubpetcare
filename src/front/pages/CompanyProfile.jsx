...
export const CompanyProfile = () => {
  ...
  const [hours, setHours] = useState("Emergency 24-hours.");
  const [about, setAbout] = useState("");      // Estado añadido para "about"
  const [address, setAddress] = useState("");  // Estado añadido para "address"
  ...
  const fetchAppointments = async () => {
    try {
      const response = await fetch(
        import.meta.env.VITE_BACKEND_URL + "/api/company/appointments",
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  // Se reemplaza la referencia a 'business' por 'store.profile'
  const getProfileImage = () => {
    if (store.profile?.photoFile) {
      return URL.createObjectURL(store.profile.photoFile);
    }
    if (store.profile && store.profile.photo && !store.profile.photoFile) {
  ...
  return (
    <div style={{ backgroundColor: "#EDF6F9" }}>
      <div className="container py-5">
        ...
        <header
          className="text-white text-center py-2 rounded"
        ...
        >
          {editing ? (
            ...
            <h1 className="display-5 open-sans-body">
              {store.profile?.name_company || "Company Name"}
            </h1>
          )}
        </header>
        ...
        <div className="my-3 p-3 bg-body rounded shadow-sm">
          <h2
            className="border-bottom pb-2 mb-0 open-sans-body"
            style={{ color: "#006D77" }}
          >
          ...
          <div className="d-flex text-body-secondary pt-3">
            <i
              className="fa-solid fa-shield-cat me-3"
              style={{ color: "#E29578" }}
            ></i>
            <p className="pb-3 mb-0 small lh-sm border-bottom sour-gummy-head">
              <strong className="d-block text-gray-dark sour-gummy-head">
                @username
              </strong>
        ...
        <section className="bg-white mt-4 p-3 rounded shadow-sm">
          <h2 className="open-sans-body" style={{ color: "#006D77" }}>
            <i
              className="fa-solid fa-briefcase-medical me-1"
              style={{ color: "#006D77" }}
            ></i>
            About Us
          </h2>
          <p className="sour-gummy-head">
            {store.profile?.description || "Company description"}
          </p>
        </section>
        ...
        <section className="bg-white mt-4 p-3 rounded shadow-sm">
          ...
          <h2 className="open-sans-body" style={{ color: "#006D77" }}>
            <i
              className="fa-solid fa-paw me-1"
              style={{ color: "#006D77" }}
            ></i>
            Our Services
          </h2>
          <div className="list-group sour-gummy-head text-muted">
            {store.services_company.map((element) => (
              <ListCompanyServices
        ...
        <section className="bg-white mt-4 p-3 rounded shadow-sm">
          ...
          {editing ? (
            ...
            <p className="sour-gummy-head">{hours}</p>
          )}
        </section>
        ...
        <section className="bg-white mt-4 p-3 rounded shadow-sm">
          ...
          <p className="sour-gummy-head">
            {store.profile?.location || "Company location"}
          </p>
        </section>
        ...
        <section className="bg-white mt-4 p-4 rounded shadow-sm">
          ...
          {store.appointments?.length > 0 ? (
            ...
            <p className="text-muted open-sans-body">
